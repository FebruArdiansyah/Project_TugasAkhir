...
class DetailBarangKeluarScreen extends StatefulWidget {
  const DetailBarangKeluarScreen({super.key});

  @override
  State<DetailBarangKeluarScreen> createState() =>
      _DetailBarangKeluarScreenState();
}

class _DetailBarangKeluarScreenState extends State<DetailBarangKeluarScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (args is Map && args['id'] != null) {
      outboundId = int.tryParse(args['id'].toString());
    }
    ...
    if (outboundId == null) {
      setState(() {
        errorMessage = 'ID barang keluar tidak ditemukan.';
      });
      return;
    }
  ...
  Future<void> _loadDetail() async {
    if (isLoading || outboundId == null) return;
    ...
    try {
      final response = await ApiService.get('/outbounds/$outboundId');

      if (response is! Map<String, dynamic>) {
        throw ApiException(
          message: 'Response detail barang keluar tidak valid.',
        );
      }
      ...
      if (data is! Map<String, dynamic>) {
        throw ApiException(
          message: 'Data detail barang keluar tidak valid.',
        );
      }
      ...
      setState(() {
        errorMessage = 'Gagal memuat detail barang keluar: $e';
      });
      _showSnackBar('Gagal memuat detail barang keluar: $e', isError: true);
    } finally {
      if (mounted) {
        setState(() {
          isLoading = false;
  ...
  void _openEditPage() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => const TambahBarangKeluarScreen(isEdit: true),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            onPressed: outboundId == null ? null : _loadDetail,
            icon: const Icon(
  ...
  Widget _buildDaftarBarangSection() {
    return _buildSection(
      color: const Color(0xFFEF4444),
        ...
        children: [
          if (items.isEmpty)
            ...
            ...items.asMap().entries.map((entry) {
              ...
              final remainingStock = _toDouble(
                item['remaining_stock'] ??
                    item['stock_after'] ??
                    item['available_stock_after'] ??
              ...
              return Padding(
                ...
                child: _BarangKeluarItemCard(
                  ...
                  kode: productCode,
                  nama: productName,
                  ukuran: sizeText.isEmpty ? '-' : sizeText,
                  gudang: warehouseName,
                  qty: _formatQty(qty),
                  satuan: unitName,
                  sisaStok: '${_formatQty(remainingStock)} $unitName',
                ),
              );
            }),
          const SizedBox(height: 10),
          _TotalBarangKeluarBox(
            totalItem: '${items.length} Item',
            totalQty: '${_formatQty(_toDouble(summary['total_qty']))} PCS',
          ),
          const SizedBox(height: 10),
          _FullInfoBoxKeluar(
  ...
  Widget _buildCatatanSection() {
    return _buildSection(
      ...
      title: 'CATATAN',
      child: _FullInfoBoxKeluar(
        title: 'Catatan',
        value: _cleanText(detail?['note']).isEmpty
            ? '-'
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: _SmallInfoKeluar(label: 'Kode Barang', value: kode),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: _SmallInfoKeluar(label: 'Ukuran', value: ukuran),
              ),
            ],
          ),
          const SizedBox(height: 8),
          Row(
            children: [
              Expanded(
                child: _SmallInfoKeluar(label: 'Gudang', value: gudang),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: _SmallInfoKeluar(
                  label: 'Sisa Stok',
                  value: sisaStok,
...
class _SmallInfoKeluar extends StatelessWidget {
  final String label;
  final String value;
  final Color valueColor;

  const _SmallInfoKeluar({
    required this.label,
    required this.value,
...
class _TotalBarangKeluarBox extends StatelessWidget {
  final String totalItem;
  final String totalQty;

  const _TotalBarangKeluarBox({
...
class _InfoTwoColumnKeluar extends StatelessWidget {
  final List<String> leftItems;
  final List<String> rightItems;

  const _InfoTwoColumnKeluar({
...
class _StatusBadgeKeluar extends StatelessWidget {
  final String text;
  final Color textColor;
  final Color bgColor;

  const _StatusBadgeKeluar({
    required this.text,
...
class _FullInfoBoxKeluar extends StatelessWidget {
  final String title;
  final String value;
